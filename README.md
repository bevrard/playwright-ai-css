...
Pour les sélecteurs ambigus observés, `css:complete` reconstruit le DOM capturé et compare les nœuds ciblés avant et après encapsulation Angular. Si le sélecteur IA ne vise plus les mêmes nœuds, le script essaie les découpages aux frontières descendantes : le préfixe ancestral va dans `:host-context(...)`, tandis que les frères et descendants restent après `:host`. Une seule conversion équivalente est acceptée ; sinon la publication est bloquée. Le rapport `selectorVerification` liste les corrections, les cas non résolus, les règles non observées et les sélecteurs hors du périmètre de ce contrôle. Les états ou sous-arbres absents de la collecte ne sont pas prouvés par cette comparaison.

`css:complete` teste la compilation Angular des règles générées. Si une règle sur `:host` bloque le compilateur, elle est d’abord réécrite avec `@scope (chemin complet)` dans `COMPOSANT.css`. Cette syntaxe demande un navigateur compatible avec CSS `@scope`. Si cette forme échoue également, la règle est déplacée vers `COMPOSANT.context.css`, à charger comme feuille **globale**. Le rapport `angularCompilation` distingue les règles réécrites (`scoped`) et déplacées (`moved`). Une règle problématique qui cible un élément interne reste signalée comme non résolue et bloque la publication. La compilation seule ne vérifie pas le comportement de la page au runtime.
